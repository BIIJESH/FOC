...
def write_bill(username, phone_number, address, venues_info, ultimate_total):
    ...
    with open("rental_bill.txt", "w") as bill_file:
        ...
        for info in venues_info:
            ...
            bill_file.write(f"Rental Period: {', '.join(str(period) for period in rental_periods)} months\n")
            bill_file.write(f"Price (excluding VAT): Rs.{', '.join(str(price) for price in total_price)}\n")
            bill_file.write(f"VAT (15%): Rs.{total_vat}\n")
            bill_file.write(f"Final Amount (including VAT): Rs.{final_amount}\n\n")

        bill_file.write(f"Ultimate Total (including VAT) for all venues: Rs.{ultimate_total}\n\n")

def formatbill(username,phone_number,locations,rental_periods,address,venues_info):
    bill_top=( f"""
